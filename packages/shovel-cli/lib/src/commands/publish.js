...
const confirmVersion = async (cur, exp) => {
...
const publishPnpm = async (preRelease) => {
...
const pushGit = async (version, remote = 'origin') => {
...
export const publish = async (options) => {
    try {
        ...
        let expVer = semVer.inc(curVer, type, `alpha.${Date.now()}`);
        expVer = isPreRelease ? expVer === null || expVer === void 0 ? void 0 : expVer.slice(0, -2) : expVer;
